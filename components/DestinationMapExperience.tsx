...
import { StopReviewCard } from "@/components/StopReviewCard";

type DestinationMapExperienceProps = {
  itinerary: Itinerary;
  mapboxToken: string | undefined;
  showReviewUI: boolean;
  onItineraryComplete: (answers: Record<string, string[]>) => void;
};
...
export function DestinationMapExperience({
  ...
  const [answersByStopId, setAnswersByStopId] = useState<Record<string, string[]>>({});
  ...
  const handleContinue = (answers: string[]) => {
    const stop = itinerary.stops[activeStopIndex];
    if (!stop) return;
    const merged = { ...answersByStopId, [stop.id]: answers };
