...
export function ReviewCard({
  ...
  return (
    ...
    >
      {isArrival ? (
        <>
          <p className="text-xs uppercase tracking-[0.28em] text-sky-300/90">Arrival</p>
          <h2 className="mt-1 text-2xl font-semibold text-slate-50">Welcome to {cityName}</h2>
          <p className="mt-2 text-sm leading-relaxed text-slate-300">
            You have landed. Your first glimpse is{" "}
            <span className="font-medium text-slate-100">{stopName}</span> — a{" "}
            <span className="capitalize">{stopType}</span> that sets the tone for the city ahead.
          </p>
          <p className="mt-3 text-sm text-slate-400">
            Next, explore your {cityName} itinerary on the map — one stop at a time.
          </p>
          <button
            type="button"
            onClick={onContinue}
            className="mt-6 w-full rounded-lg bg-gradient-to-r from-sky-500 to-violet-500 px-4 py-2.5 text-sm font-medium text-white transition hover:brightness-110 sm:w-auto"
          >
